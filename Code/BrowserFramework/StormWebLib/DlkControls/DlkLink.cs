       ...
       public void ClickIfExists(String TrueOrFalse)
        ...
        public void VerifyReadOnly(String TrueOrFalse)
        {
            try
            {
                ...
                if (mElement.GetAttribute("class").ToLower().Contains("disabled") || mElement.GetAttribute("class").ToLower().Contains("masked"))
                {
                    isReadOnly = true;
                }
        ...
        [Keyword("VerifyAddSymbolExist", new String[] { "1|text|Expected Value|TRUE or FALSE" })]
        public void VerifyAddSymbolExist(String TrueOrFalse)
        {
            try
            {
                Initialize();
                bool isVisible = (mElement.FindElements(By.XPath("./preceding-sibling::span[contains (@class, 'add-symbol')]")).Where(item => item.Displayed).ToList().Count) > 0;
                DlkAssert.AssertEqual("VerifyAddSymbolExist()", Convert.ToBoolean(TrueOrFalse), isVisible);
            }
            catch (Exception ex)
            {
                throw new Exception("VerifyAddSymbolExist failed : ", ex);
            }
        }
    }
}
