        ...
        new public String GetValue()
        {
            ...
            try
            {
                if (true) // replace 'true' with condition that checks if parsing is allowed in settings or in some file
                {
                    ...
                    // try to parse immediately, maybe the localized string has no placeholder
                    english = ParseToEnglishByDictionaryKey(GetForeignDictionaryKeyByValue(elementText));
                    if (String.IsNullOrWhiteSpace(english))
                    {
                        // placeholder substitution technique (lol i just made that up, there's no such thing)
                        ...
                        // this array contains the values for the standard placeholders such as {0}
                        var standard = placeholderValue.Item1;
                        // while this array contains the values for the custom placeholders such as [WBS]
                        ...
                        if (standard.Count() != 0)
                        {
                            // "flatten" the unformatted string using the placeholder values
                            var substituted = ReplacePlaceholderValue(english, standard);
                            // return spanish string if not applicable
                            english = String.IsNullOrWhiteSpace(substituted) ? elementText : substituted;
                        }
                        if (wbs.Count() != 0)
                        {
                            // TODO: ADD LOGIC THAT WILL SUBSTITUTE CUSTOM PLACEHOLDERS TO PLACEHOLDER VALUES
