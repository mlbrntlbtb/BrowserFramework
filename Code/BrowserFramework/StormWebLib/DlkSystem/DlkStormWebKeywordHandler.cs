...
namespace StormWebLib.System
{
    ...
    /// </summary>
    public static class DlkStormWebKeywordHandler
    {
        ...
        public static void ExecuteKeyword(String Screen, String ControlName, String Keyword, String[] Parameters)
        {
            ...
            StormWebLib.DlkControls.DlkImage mImage;
            DlkToolTip mToolTip;
            ...
            DlkSlider mSlider;
            DlkTimeline mTimeline;
            DlkReportViewer mReport;
            ...
            switch (mControl.mControlType.ToLower())
            {
                ...
                case "uidialog":
                    ...
                    break;
                case "image":
                    mImage = new StormWebLib.DlkControls.DlkImage(mControl.mKey, mControl.mSearchMethod, mControl.mSearchParameters.Split('~'));
                    switch (Parameters.Length)
                    {
                        case 1:
                            if (Parameters[0] == "")
                ...
                case "calendar":
                    ...
                    break;
                case "timeline":
                    mTimeline = new DlkTimeline(mControl.mKey, mControl.mSearchMethod, mControl.mSearchParameters.Split('~'));
                    switch (Parameters.Length)
                    {
                        case 1:
                            if (Parameters[0] == "")
                            {
                                try
                                {
                                    mi = mTimeline.GetType().GetMethod(Keyword, new Type[] { });
                                    mi.Invoke(mTimeline, null);
                                }
                                catch
                                {
                                    mi = mTimeline.GetType().GetMethod(Keyword, new Type[] { typeof(String) });
                                    mi.Invoke(mTimeline, new[] { Parameters[0] });
                                }
                            }
                            else
                            {
                            ...
                            break;
                        case 2:
                            mi = mTimeline.GetType().GetMethod(Keyword, new Type[] { typeof(String), typeof(String) });
                            mi.Invoke(mTimeline, new[] { Parameters[0], Parameters[1] });
                            break;
                        case 3:
                            mi = mTimeline.GetType().GetMethod(Keyword, new Type[] { typeof(String), typeof(String), typeof(String) });
                            mi.Invoke(mTimeline, new[] { Parameters[0], Parameters[1], Parameters[2] });
                            break;
                        default:
                            throw new Exception("Unsupported keyword: " + Keyword);
                    }
                    break;
                case "reportviewer":
                    mReport = new DlkReportViewer(mControl.mKey, mControl.mSearchMethod, mControl.mSearchParameters.Split('~'));
                    switch (Parameters.Length)
                    {
                        case 1:
                            if (Parameters[0] == "")
                            {
                                try
                                {
                                    mi = mReport.GetType().GetMethod(Keyword, new Type[] { });
                                    mi.Invoke(mReport, null);
                                }
                                catch
                                {
                                    mi = mReport.GetType().GetMethod(Keyword, new Type[] { typeof(String) });
                                    mi.Invoke(mReport, new[] { Parameters[0] });
                                }
                            }
                            else
                            {
                            ...
                            break;
                        case 2:
                            mi = mReport.GetType().GetMethod(Keyword, new Type[] { typeof(String), typeof(String) });
                            mi.Invoke(mReport, new[] { Parameters[0], Parameters[1] });
                            break;
                        default:
                            throw new Exception("Unsupported keyword: " + Keyword);
                    }
