...
{
    public class DlkAcumenTouchStoneTestExecute : DlkTestExecute
    {
        public DlkAcumenTouchStoneTestExecute(String Product, String LoginConfig, String TestPath, String TestInstance, String Browser, String KeepBrowserOpen)
        ...
        public override void TestSetup()
        {
            try
            {
                string buildFile = @"\\ashv0193\Deltek\Acumen Touchstone\web\app\ProductBuild.xml";
                base.TestSetup(); // common setup for a test
                if (File.Exists(buildFile))
                {
                    if (!DlkEnvironment.CustomInfo.Keys.Contains("buildversion"))
                    {
                        string buildVersion = "";
                        string gaNumber = "";
                        string cuNumber = "";
                        ...
                        XDocument buildDoc = XDocument.Load(buildFile);
                        var buildElement = buildDoc.Descendants("SoftwareVersion").FirstOrDefault();
                        if (buildElement != null)
                        {
                            buildVersion = buildElement.Value;
                            if (buildVersion.Contains("."))
                            {
                                string[] releaseVersions = buildVersion.Split('.');
                                gaNumber = releaseVersions[0] + "." + releaseVersions[1];
                                cuNumber = releaseVersions[0] + "." + releaseVersions[1] + "." + releaseVersions[2];
                                buildNumber = releaseVersions[3];
                            }
                        }
                        DlkEnvironment.CustomInfo.Add("buildversion", new string[] { "Build Version", buildVersion });
                        DlkEnvironment.CustomInfo.Add("ga", new string[] { "GA", gaNumber });
                        DlkEnvironment.CustomInfo.Add("cu", new string[] { "CU", cuNumber });
                        DlkEnvironment.CustomInfo.Add("build", new string[] { "Build", buildNumber });
                    }
                }
                    
                DlkAcumenTouchStoneFunctionHandler.ExecuteFunction("Login", "Function", "Login",
            ...
            for (int i = 0; i < mTest.mTestSteps.Count; i++) // for each test step
            {
                try
                {
                    ...
                    else if (execute)
                    {
                        ...
                        {
                            DlkAcumenTouchStoneFunctionHandler.ExecuteFunction
                                (
                        ...
                        {
                            DlkAcumenTouchStoneKeywordHandler.ExecuteKeyword(
                                mTest.mTestSteps[i].mScreen,
                                mTest.mTestSteps[i].mControl,
                                mTest.mTestSteps[i].mKeyword,
                                mTestParams
