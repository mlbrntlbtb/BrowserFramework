...
namespace AcumenTouchStoneLib.DlkControls
{
    ...
    public class DlkTable : DlkBaseTable
    {
        ...
        public class Table
        {
            ...
            public bool IsFirstCellButtonClick = true;
            public int RetryClick = 0;
            ...
            private void SetAndEnterTextColValue(IWebElement Cell, string Value)
            {
                    ...
                    !String.IsNullOrEmpty(mTextElm.Text))
                {
                    ...
                    if (!String.IsNullOrEmpty(mTextElm.GetAttribute("value")) ||
                        !String.IsNullOrEmpty(mTextElm.Text))
                    {
            ...
            {
                IWebElement mCheckBoxElm = Cell.FindElements(By.CssSelector("span.gridCheckbox")).Where(x => x.Displayed).Any() ?
                    Cell.FindElement(By.CssSelector("span.gridCheckbox")) : null;

                if (mCheckBoxElm == null)
                {
                    DlkLogger.LogInfo("Checkbox not found. Trying again... ");
                    mCheckBoxElm = Cell.FindElements(By.CssSelector("span.checkbox")).Where(x => x.Displayed).Any() ?
                    Cell.FindElement(By.CssSelector("span.checkbox")) : throw new Exception("Checkbox not found.");
                }
            ...
            private void SetCheckBoxColValue(IWebElement Cell, string Value)
            {
                ...
                if(mCheckBoxElm != null)
                {
                    DlkLogger.LogInfo("Checkbox found.");
                    DlkCheckBox chkCell = new DlkCheckBox("CheckBox", mCheckBoxElm);
                    chkCell.ScrollIntoViewUsingJavaScript(true);
                    chkCell.Set(Value);
                    for (int retryCount = 0; retryCount <= 2; retryCount++)
                    {
                        DlkLogger.LogInfo("Checkbox click done. Checking if value was saved...");
                        bool isChecked = chkCell.GetCheckedState();
                        DlkLogger.LogInfo("Checkbox value: [" + isChecked.ToString() + "]");
                        if (isChecked == Convert.ToBoolean(Value))
                        {
                            DlkLogger.LogInfo("Checkbox value changed. Proceeding...");
                            break;
                        }
                        else
                        {
                            DlkLogger.LogInfo("Checkbox value unchanged. Retrying click...");
                            Thread.Sleep(3000);
                            chkCell.ScrollIntoViewUsingJavaScript(true);
                            chkCell.Set(Value);
                        }
                        if (retryCount == 2)
                        {
                            DlkLogger.LogInfo("Checkbox has been re-clicked 3 times.");
                        }
                    }
                }
            }
            ...
            public void SetCellValue(int iRow, string sColumnHeader, string sValue)
            {
                ...
                DlkBaseControl mCell = new DlkBaseControl("Cell", mCellElm);
                mCell.Click();
                string sClass = mCellElm.GetAttribute("class");

                if (sClass == "")
                {
                    IWebElement chkBoxCell = mCellElm.FindElement(By.XPath(".//div[contains(@class, 'cell')]"));
                    sClass = chkBoxCell.GetAttribute("class");
                }
                ...
                else
                {
                    ...
                    else if (sClass.Contains("core-component"))
                    {
            ...
            public void SetFilterCellValue(string sColumnHeader, string sValue)
            {
                IWebElement filterRow = null;
                if (mTableElement.FindElements(By.XPath(".//tr[contains(@class,'filterRow')]")).Any())
                {
                    IWebElement hdrFilterRow = mTableElement.FindElement(By.XPath(".//tr[contains(@class,'filterRow')]"));
                    if (hdrFilterRow.Displayed)
                    {
                        filterRow = hdrFilterRow;
                    }
                }
                if (filterRow == null)
                {
                ...
                IList<IWebElement> cells = filterRow.FindElements(By.CssSelector("td")).Count > 0 ? filterRow.FindElements(By.CssSelector("td")).Where(x => x.GetCssValue("display").ToLower().Trim() != "none").ToList() : null;
                IWebElement mCellElm = cells[iCol];
                if (mCellElm == null)
                    throw new Exception("SetCellValue() failed. Invalid row.");
                DlkBaseControl mCell = new DlkBaseControl("Cell", mCellElm);
                mCell.Click();
            ...
            public void MoveCellSlider(int iRow, string sColumnHeader, string sDirection, string sValue)
            {
                IWebElement mCellElm = GetCell(iRow, sColumnHeader);
                if (mCellElm == null)
                    throw new Exception("SetCellValue() failed. Invalid row.");
                DlkBaseControl mCell = new DlkBaseControl("Cell", mCellElm);
                string sClass = mCell.GetAttributeValue("class");
                IWebElement slider = mCellElm.FindElements(By.XPath(".//div[contains(@class, 'ui-slider-horizontal')]")).First();
                mCell.ScrollIntoViewUsingJavaScript(true);
                DlkSlider sliderControl = new DlkSlider("Slider", slider);
                sliderControl.SetValue(sDirection, sValue);
                Thread.Sleep(500);
                ClickBanner();
            }

            public void VerifyCellSliderValue(int iRow, string sColumnHeader, string sValue)
            {
                IWebElement mCellElm = GetCell(iRow, sColumnHeader);
                if (mCellElm == null)
                    throw new Exception("SetCellValue() failed. Invalid row.");
                ...
                mCell.ScrollIntoViewUsingJavaScript(true);
                DlkSlider sliderControl = new DlkSlider("Slider", slider);
                sliderControl.VerifyValue(sValue);
            }
            ...
            public void VerifyCellTextColor(int iRow, string sColumnHeader, string sExpectedValue)
            {
                IWebElement mCellElm = GetCell(iRow, sColumnHeader);
                if (mCellElm == null)
                    throw new Exception("VerifyCellTextColor() failed. Invalid row.");

                DlkBaseControl mCell = new DlkBaseControl("Cell", mCellElm);
                mCell.ScrollIntoViewUsingJavaScript(true);
                if (mCellElm.FindElements(By.XPath(".//*[contains(@class,'cellLink')]")).Any())
                {
                    mCellElm = mCellElm.FindElement(By.XPath(".//*[contains(@class,'cellLink')]"));
                }
                else if (mCellElm.FindElements(By.XPath(".//span")).Any())
                {
                    mCellElm = mCellElm.FindElement(By.XPath(".//span"));
                }
                string fontColor = mCellElm.GetCssValue("color");
                if (DlkEnvironment.mBrowser.ToLower() == "firefox")
                {
                    fontColor = fontColor.Replace(")", ", " + mCellElm.GetCssValue("opacity") + ")").Replace("rgb", "rgba");
                }
                DlkAssert.AssertEqual("VerifyCellTextColor() : ", sExpectedValue, fontColor);
            }
            ...
            public void VerifyCellButtonExists(int iRow, string sColumnHeader, string ExpectedValue)
            {
                bool actualValue = false, expectedValue = Convert.ToBoolean(ExpectedValue);

                IWebElement mCellElm = GetCell(iRow, sColumnHeader);
                if (mCellElm == null)
                    throw new Exception("VerifyCellButtonExists() failed. Invalid row.");
                if (mCellElm.FindElements(By.TagName("button")).Where(item => !item.GetAttribute("style").Contains("display: none")).Count() > 0)
                {
                    actualValue = true;
                }
                else if (mCellElm.FindElements(By.XPath(".//*[contains(@class, 'treeValueIcon')]")).Where(item => !item.GetAttribute("style").Contains("display: none")).Count() > 0)
                {
                ...
                DlkAssert.AssertEqual("VerifyCellButtonExists() : ", expectedValue, actualValue);
            }
            ...
            public void ClickCellButton(int iRow, string sColumnHeader, string sButtonCaption)
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        if (mCellElm.FindElements(By.TagName("button")).Where(item => !item.GetAttribute("style").Contains("display: none")).Count() > 0)
                        {
                            mCellButton = mCellElm.FindElement(By.XPath(String.Format(".//button[@title='{0}']", sButtonCaption)));
                        }
                        ...
                        else if (sButtonCaption == "")
                        {
                            // buttons with no caption
                            mCellButton = mCellElm.FindElement(By.XPath(".//*[contains(@class, 'treeValueIcon')]"));
                        }
                        else
                        {
                        ...
                        {
                            DlkLogger.LogInfo("Cell button found. Clicking on button... ");
                        ...
                        else
                        {
                            if (mTableElement.GetAttribute("class").Contains("rowTools"))
                            {
                                //IWebElement mainTable = mTableElement.FindElement(By.XPath("../preceding-sibling::div[contains(@class,'gridBody')]//table[@class='bodyTable']"));
                                //List<IWebElement> mainRows = mainTable.FindElements(By.XPath("..//tr")).ToList();
                                //mainRows[iRow - 1].Click();
                                //ctlCellButton.MouseOver();
                                if (sButtonCaption == "Email")
                                {
                                    Thread.Sleep(2000);
                                    mCellElm = GetCell(iRow, sColumnHeader);
                                    mCellButton = mCellElm.FindElement(By.XPath(String.Format(".//button[@title='{0}']", sButtonCaption)));
                                    ctlCellButton = new DlkBaseControl("Cell Button", mCellButton);
                                    DlkLogger.LogInfo("Row tool button found. Clicking on button... ");
                                    TryClickElement(ctlCellButton);
                                    ClickBanner();
                                    return;
                                }
                                try
                                {
                                    DlkLogger.LogInfo("Row tool button found. Clicking on button [1]... ");
                                    ctlCellButton.ClickUsingJavaScript();
                                    Thread.Sleep(1000);
                                }
                                catch
                                {
                                    DlkLogger.LogInfo("Error on clicking on row tool button twice. Click button failed.");
                                }
                            }
                            else
                            {
                    ...
                    {
                        DlkLogger.LogInfo("Error on clicking on cell button. Click button failed.");
            ...
            public void ClickCellButtonWithMenu(int iRow, string sColumnHeader, string sButtonCaption, string sMenuItem)
            {
                try
                {
                    DlkBaseControl ctlCellButton;
                    ...
                    try
                    {
                        ...
                        if (DlkEnvironment.AutoDriver.FindElements(By.XPath("//div[contains(@class, 'popupmenu')]")).Where(x => x.Displayed).Any())
                        {
                            IWebElement menuElmt = DlkEnvironment.AutoDriver.FindElements(By.XPath("//div[contains(@class, 'popupmenu')]")).Where(x => x.Displayed).First();
                            List<IWebElement> menuItems = new List<IWebElement>();
                            menuItems = GetItems(menuElmt);
                            bool bFound = false;

                            foreach (IWebElement aButton in menuItems)
                            {
                                DlkBaseControl menuItem = new DlkBaseControl("MenuItem", aButton);
                                if (menuItem.GetValue().Trim().ToLower() == sMenuItem.Trim().ToLower())
                                {
                                    if (menuElmt.GetAttribute("class") == "MenuBarBkGnd")
                                    {
                                        menuItem.Click();
                                    }
                                    else
                                    ...
                                    if (IsFirstCellButtonClick)
                                    {
                                        if (sMenuItem.ToLower() == "delete")
                                        {
                                            System.Threading.Thread.Sleep(2000);
                                            DlkLogger.LogInfo("Looking for Delete dialog...");
                                            while (!DlkEnvironment.AutoDriver.FindElements(By.XPath("//div[@aria-describedby='mbDiv']")).Where(x => x.Displayed).Any())
                                            {
                                                if (DlkEnvironment.AutoDriver.FindElements(By.XPath("//div[@aria-describedby='mbDiv']")).Where(x => x.Displayed).Any())
                                                {
                                                    DlkLogger.LogInfo("Dialog for delete found");
                                                    break;
                                                }
                                                else
                                                {
                                                    IsFirstCellButtonClick = false;
                                                    RetryClick++;
                                                }
                                            }
                                        }
                            ...
                            if (!bFound)
                            {
                                throw new Exception("Menu item '" + sMenuItem + "' not found");
                            }
                        }
                    }
                    catch (NoSuchElementException)
            ...
            public IWebElement GetCellByContent(string sCellContent, string sColumnHeader)
            {
                int iCol = GetColumnIndex(sColumnHeader);
                IList<IWebElement> cells = mTableElement.FindElements(By.CssSelector("td")).Count > 0 ? mTableElement.FindElements(By.CssSelector("td")).Where(x => x.GetCssValue("display").ToLower().Trim() != "none").Where(x => x.GetAttribute("innerText") == sCellContent).ToList() : null;
                if (cells == null)
                {
                    return null;
                }
            ...
            public IWebElement GetRow(int iRow)
            {
                try
                {
                    ...
                    if (mTableElement.FindElements(By.XPath(".//table[contains(@class, 'rowToolsTable')]")).Count > 0)
                    {
            ...
            public bool VerifyTableTitle(String TableTitle)
            {
                bool bFound = false;
                string mTitle = "";
                IList<IWebElement> mTitleControls = mTableElement.FindElements(By.XPath(".//descendant::*[@class='caption truncate_text']"));
                if (mTitleControls.Count > 0)
                {
                    mTitle = mTitleControls.First().GetAttribute("title");
                    if (String.Equals(mTitle, TableTitle, StringComparison.InvariantCultureIgnoreCase))
                    {
                        bFound = true;
                    }
                }
            ...
            public IWebElement GetColumnHeader(String ColumnHeaderValue)
            {
                ...
                while (!bFound && (curRetry++ <= retryLimit))
                {
                    foreach (IWebElement columnHeader in columnHeaders)
                    {
                        if (!String.IsNullOrEmpty(columnHeader.GetAttribute("name")) && curRetry == 1)
                        {
                            columnHeaderValue = columnHeader.GetAttribute("name");
                        }
                        else if (!String.IsNullOrEmpty(columnHeader.GetAttribute("title")) && curRetry == 2)
                        {
                            columnHeaderValue = columnHeader.GetAttribute("title");
                        }
                        else
                        ...
                        if (columnHeaderValue.ToLower() == ColumnHeaderValue.ToLower())
                        {
                ...
                try
                {
                  ...
                  , By.XPath(".//div[@contenteditable='true']"));
                    ...
                    if (mTextElm != null)
                    {
                        //Clear the initial contents of the cell
                        //In few instances, mElement.Clear() disables the control
                        DlkLogger.LogInfo("Clearing current cell value... ");
                        ClearField(mTextElm);
                        ...
                        DlkLogger.LogInfo("Setting value [" + Value + "] to target cell... ");
                        if (!String.IsNullOrEmpty(Value))
                            mTextElm.SendKeys(Value);
                        else
            ...
            public void ExpandRow(String rowNumber)
            {
                int rowNum;
                try
                {
                    rowNum = Convert.ToInt32(rowNumber);
                    rowNum = rowNum - 1;
                }
                catch (Exception e)
                {
                    throw new Exception("ExpandRow() failed : " + e.Message, e);
                }
                /*The list below will contain all the conditions that define the html buttons
                 * that will included in the list of webelements to be processed in this keyword.
                 * Any new conditions that needs to be considered must be added in the list below using the proper and working xpath.
                */
                ...
                xpath_conditions.Add("[not(contains(@class,'add-button'))]");
                xpath_conditions.Add("[not(contains(@class,'rowTools'))]");
                xpath_conditions.Add("[not(contains(@class,'childRow'))]");
                ...
                {
                    //for rows not using button to expand
                    buttonsInTbl = mTableElement.FindElements(By.XPath(".//span[contains(@class,'tree-wrap')]")).ToList();

                    if (buttonsInTbl.Count == 0)
                    {
                        //for ngRP
                        IList<IWebElement> rowsInTbl = mTableElement.FindElements(By.XPath(".//table[@class='bodyTable']//tr[not(contains(@style,'display: none'))][not(contains(@style,'display:none'))]")).ToList();
                        foreach (IWebElement tblRow in rowsInTbl)
                        {
                            if (tblRow.FindElements(By.XPath(".//div[contains(@class,'treeIcon')][not(contains(@class,'noChildren'))]")).Any())
                            {
                                buttonsInTbl.Add(tblRow.FindElement(By.XPath(".//div[contains(@class,'treeIcon')][not(contains(@class,'noChildren'))]")));
                            }
                            else
                            {
                                buttonsInTbl.Add(null);
                            }
                        }
                        if (buttonsInTbl[rowNum] == null)
                        {
                            throw new Exception("ExpandRow() failed : row doesn't have expand button");
                        }
                        try
                        {
                            DlkImage mImgTmp = new DlkImage("ExpandImageLink", buttonsInTbl[rowNum]);
                            String mSrc = buttonsInTbl[rowNum].GetAttribute("class");
                            if (mSrc.Contains("treeIcon"))
                            {
                                //if (!mSrc.Contains("expanded") && !mSrc.Contains("noChildren"))
                                if (!mSrc.Contains("expanded"))
                                {
                                    mImgTmp.ScrollIntoViewUsingJavaScript(true);
                                    buttonsInTbl[rowNum].Click();
                                    DlkLogger.LogInfo("Successfully expanded row " + rowNumber);
                                    Thread.Sleep(1000);
                                }
                            }
                        }
                        catch
                        {
                            //object reference not set, refresh contents of buttonsInTbl
                            DlkAcumenTouchStoneFunctionHandler.WaitScreenGetsReady();
                            buttonsInTbl = mTableElement.FindElements(By.XPath(".//div[contains(@class,'treeIcon')][not(contains(@class,'noChildren'))]"));
                            DlkImage mImgTmp = new DlkImage("ExpandImageLink", buttonsInTbl[rowNum]);
                            String mSrc = buttonsInTbl[rowNum].GetAttribute("class");
                            if (mSrc.Contains("treeIcon"))
                            {
            ...
            public void ExpandRowByName(String rowName)
            {
                IWebElement buttonToClick = null;
                IWebElement row = null;
                IList<IWebElement> rowsInTbl = null;
                string allRowsTable_XPath1 = ".//tr[contains(@class,'gotChildren') or contains(@class,'childRow')][not(contains(@style,'display: none'))][not(contains(@style,'display:none'))]";
               
                if (mTableElement.FindElements(By.XPath(allRowsTable_XPath1)).Any())
                {
                    rowsInTbl = mTableElement.FindElements(By.XPath(allRowsTable_XPath1)).ToList();
                    
                    string treeItem_XPath = ".//div[contains(@class,'treeValue')][text()='" + rowName + "'] | " +
                        "//span[contains(@class, 'infoBubble')][text()='" + rowName + "']";
                    bool tFound = false;
                    
                    foreach (IWebElement currentRow in rowsInTbl)
                    {
                        if(currentRow.FindElements(By.XPath(treeItem_XPath)).Any())
                        {
                            row = currentRow.FindElement(By.XPath(treeItem_XPath));
                            DlkLogger.LogInfo("Tree item [" + rowName + "] found.");
                            tFound = true;
                            break;
                        }
                    }
                    ...
                    if (!tFound)
                        throw new Exception("Tree item [" + rowName + "] not found.");
                }
                else
                {
                    rowsInTbl = mTableElement.FindElements(By.XPath(".//tbody//tr")).Where(x => x.FindElements(By.XPath(".//div[contains(@class,'treeValue')]")).Any()).ToList();
                    row = rowsInTbl.First(x => x.FindElements(By.XPath(".//span[contains(@class, 'infoBubble')][text()='" + rowName + "'] | .//*[normalize-space() = '" + rowName + "']")).Any());
                }

                string buttonToClick_XPath1 = ".//div[contains(@class,'treeIcon')][not(contains(@class,'noChildren'))]";
                string buttonToClick_XPath2 = ".//ancestor::div[contains(@class,'treeValue')]//preceding-sibling::div[contains(@class,'treeIcon')][not(contains(@class,'noChildren'))]";
                if (row.FindElements(By.XPath(buttonToClick_XPath1)).Any())
                {
                    buttonToClick = row.FindElement(By.XPath(buttonToClick_XPath1));
                }
                else if (row.FindElements(By.XPath(buttonToClick_XPath2)).Any())
                {
                    buttonToClick = row.FindElement(By.XPath(buttonToClick_XPath2));
                }

                try
                {
                    DlkImage mImgTmp = new DlkImage("ExpandImageLink", buttonToClick);
                    String mSrc = buttonToClick.GetAttribute("class");
                    if (mSrc.Contains("treeIcon"))
                    {
                        //if (!mSrc.Contains("expanded") && !mSrc.Contains("noChildren"))
                        if (!mSrc.Contains("expanded"))
                        {
                            mImgTmp.ScrollIntoViewUsingJavaScript(true);
                            buttonToClick.Click();
                            DlkLogger.LogInfo("Successfully expanded row with name " + rowName);
                            Thread.Sleep(1000);
                        }
                        else
                        {
                            DlkLogger.LogInfo("Tree icon is already expanded.");
                        }
                    }
                    else
                    {
                        DlkLogger.LogInfo("Tree icon not found. No items expanded.");
                    }
                }
                catch (Exception e)
                {
                    throw new Exception("ExpandRowByName() failed : " + e.Message, e);
                }
            }
            ...
            public void VerifyRowHighlighted(String iRow, bool TrueOrFalse)
            {
                try
                {
                    ...
                    DlkAssert.AssertEqual("VerifyRowHighlighted", TrueOrFalse, highlight);
                }
                catch (Exception e)
                {
            ...
            public void VerifyRowSelected(String iRow, bool TrueOrFalse)
            {
                try
                {
                    ...
                    DlkAssert.AssertEqual("VerifyRowSelected", TrueOrFalse, select);
                }
                catch (Exception e)
                {
            ...
            /// </summary>
            public void ClickBanner(bool skipDialogClick = false)
            {
                try
                {
            ...
            public void LoseFocus()
            {
                ...
                jse.ExecuteScript("document.activeElement.blur()", DlkEnvironment.AutoDriver.SwitchTo().ActiveElement());
                DlkLogger.LogInfo("Removing focus to current cell... ");
            }

            private List<IWebElement> GetItems(IWebElement menuElement)
            {
                List<IWebElement> mItems = new List<IWebElement>();
                try
                {
                    mItems = menuElement.GetAttribute("class") == "MenuBarBkGnd" ? menuElement.FindElements(By.XPath(".//div")).ToList() : menuElement.FindElements(By.XPath(".//a[not(contains(@style,'none'))]")).ToList();
                    ...
                    if (mItems.Count == 0)
                    {
                        ...
                        if (!list.Exists(1))
                        {
                            IWebElement mArrowDown = menuElement.FindElement(By.XPath(".//div[contains(@class,'actionsMenuIcon')]"));
                            ctlArrowDown = new DlkBaseControl("ArrowDown", mArrowDown);
                            ctlArrowDown.ClickUsingJavaScript();
                        }
                        list.FindElement();
                        mItems = list.mElement.FindElements(By.XPath("./li[contains(@class,'popupmenuItem')]")).ToList();
                ...
                {
                    mItems = menuElement.FindElements(By.TagName("span")).ToList();
                    return mItems;
                }
            }
        ...
        public class Body
        {
            ...
            public IList<IWebElement> Rows
            {
                get
                {
                    if (rows == null) rows = e.FindElements(By.CssSelector("table>tbody>tr"))
                        ...
                        .Where(x => !x.GetAttribute("style").Contains("display: none"))
            ...
            private IList<string> GetColumnHeaderLabels()
            {
                ...
                for (int i = 0; i < columnHeaders.Count; i++)
                {
                    ...
                    if (sHeader == "")
                    {
                        ...
                        else if (sHeader.Contains("<span></span>"))
                        {
                            sHeader = sHeader.Replace("<span></span>", "");
                        }
                    }
            ...
            public string GetCellValue(int rowNumber, string sColumnHeader)
            {
                ...
                if (sClass == "")
                {
                    IWebElement chkBoxCell;
                    if (cell.FindElements(By.XPath(".//div[contains(@class, 'cell')]")).Count > 0)
                    {
                        chkBoxCell = cell.FindElement(By.XPath(".//div[contains(@class, 'cell')]"));
                        sClass = chkBoxCell.GetAttribute("class");
                    }
                }
            ...
            public int GetRowWithColumnValue(string sColumnHeader, string sValue)
            {
                ...
                try
                {
                    ...
                    {
                        cellValue = GetCellValue(i, sColumnHeader);
                        DlkLogger.LogInfo("Current cell value: [" + cellValue + "]");
            ...
            {
                string cellValue = "";
                string xpath_CheckBox = ".//span[contains(@class,'checkbox')]";

                IWebElement checkBox = Cell.FindElements(By.XPath(xpath_CheckBox)).First();
        ...
        public void GetRowWithColumnValue(string ColumnHeader, string Value, string VariableName)
        {
            try
            {
                ...
                {
                    DlkLogger.LogInfo("Re-initializing due to unfound row");
                    Thread.Sleep(2000);
                    Initialize();
                    iRow = table.Body.GetRowWithColumnValue(ColumnHeader, Value);
                    DlkVariable.SetVariable(VariableName, iRow.ToString());
                }
                DlkLogger.LogInfo("Value [" + iRow.ToString() + "] assigned to Variable [" + VariableName + "]");
                //TableData.ClickBanner();
        ...
        [Keyword("SetFilterCellValue", new String[] { "1|text|Row|1",
                                                "2|text|ColumnHeader|Notes"})]
        public void SetFilterCellValue(string ColumnHeader, string Value)
        {
            Initialize();
            TableData.SetFilterCellValue(ColumnHeader, Value);
        }

        [Keyword("MoveCellSlider", new String[] { "1|text|Row|1",
                                                "2|text|ColumnHeader|Notes",
                                                "3|text|Value|sample text"})]
        public void MoveCellSlider(string Row, string ColumnHeader, string Direction, string Value)
        {
            Initialize();
            TableData.MoveCellSlider(int.Parse(Row), ColumnHeader, Direction, Value);
        }

        [Keyword("VerifyCellSliderValue", new String[] { "1|text|Row|1",
                                                "2|text|ColumnHeader|Notes",
                                                "3|text|Value|sample text"})]
        public void VerifyCellSliderValue(string Row, string ColumnHeader, string ExpectedValue)
        {
            Initialize();
            TableData.VerifyCellSliderValue(int.Parse(Row), ColumnHeader, ExpectedValue);
        }
        ...
        [Keyword("VerifyCellTextColor", new String[] { "1|text|Row|1",
                                                   "2|text|ColumnHeader|Notes",
                                                   "3|text|ExpectedValue|sample text"})]
        public void VerifyCellTextColor(string Row, string ColumnHeader, string ExpectedValue)
        {
            try
            {
                Initialize();
                TableData.VerifyCellTextColor(int.Parse(Row), ColumnHeader, ExpectedValue);
            }
            catch (Exception e)
            {
                throw new Exception("VerifyCellTextColor() failed : " + e.Message, e);
            }
        }
        ...
        [Keyword("VerifyCellButtonExists", new String[] { "1|text|Row|1",
                                                   "2|text|ColumnHeader|Notes",
                                                   "3|text|ExpectedValue|sample text"})]
        public void VerifyCellButtonExists(string Row, string ColumnHeader, string ExpectedValue)
        {
            try
            {
                Initialize();
                TableData.VerifyCellButtonExists(int.Parse(Row), ColumnHeader, ExpectedValue);
            }
            catch (Exception e)
            {
                throw new Exception("VerifyCellButtonExists() failed : " + e.Message, e);
            }
        }
        ...
        [Keyword("ClickCellButtonWithMenu", new String[] { "1|text|Row|1",
                                                  "2|text|ColumnHeader|Notes",
                                                  "3|text|ButtonCaption|Option",
                                                  "4|text|MenuItem|Edit"})]
        public void ClickCellButtonWithMenu(string Row, string ColumnHeader, string Caption, string MenuItem)
        {
            Initialize();
            TableData.ClickCellButtonWithMenu(int.Parse(Row), ColumnHeader, Caption, MenuItem);
            if (!TableData.IsFirstCellButtonClick && TableData.RetryClick <= 100)
            {
                DlkLogger.LogInfo("Dialog unavailable. Retry attempt number " + TableData.RetryClick.ToString());
                TableData.IsFirstCellButtonClick = true;
                Initialize();
                TableData.ClickCellButtonWithMenu(int.Parse(Row), ColumnHeader, Caption, MenuItem);
            }
        }
        ...
        public void ClickCellLink(string Row, string ColumnHeader, string LinkText)
        {
            ...
            {
                if (e.Message.Contains("is stale"))
                {
                    try
                    {
                        DlkLogger.LogInfo("Table has changed, reloading");
                        Initialize();
                        TableData.ClickCellLink(int.Parse(Row), ColumnHeader, LinkText);
                    }
                    catch
        ...
        public void DoubleClickCell(String Row, String ColumnHeader)
        {
            try
            {
                Initialize();
        ...
        [Keyword("ClickCellByContent", new String[] { "1|text|Expected Value|TRUE" })]
        public void ClickCellByContent(String CellContent, String ColumnHeader)
        {
            try
            {
                Initialize();
                IWebElement targetCell = TableData.GetCellByContent(CellContent, ColumnHeader);
                //IWebElement targetLink = targetCell.FindElement(By.XPath("./descendant::div[@class='inputContainer']/input[1]"));
                DlkBaseControl target = new DlkBaseControl("TargetCell", targetCell);
                target.ScrollIntoViewUsingJavaScript(true);
                DlkAcumenTouchStoneFunctionHandler.WaitScreenGetsReady();
        ...
        [Keyword("ExpandRow", new String[] { "1|text|Expected Value|Row number"})]
        public void ExpandRow(String RowNumber)
        {
            try
            {
                Initialize();
                TableData.ExpandRow(RowNumber);
            }
            catch (Exception e)
            {
                if (e.Message.Contains("is stale"))
                {
                    ...
                    {
                        throw new Exception("ExpandRow() failed : " + ex.Message, ex);
                    }
                }
                throw new Exception("ExpandRow() failed : " + e.Message, e);
            }
        ...
        [Keyword("ExpandRowByName", new String[] { "1|text|Expected Value|Row name" })]
        public void ExpandRowByName(String RowName)
        {
            try
            {
                Initialize();
                TableData.ExpandRowByName(RowName);
            }
            catch (Exception e)
            {
                throw new Exception("ExpandRow() failed : " + e.Message, e);
        ...
        [Keyword("VerifyTableTitle", new String[] { "1|text|Expected Value|TRUE" })]
        public void VerifyTableTitle(String TableTitle, String TrueOrFalse)
        {
            Initialize();
            DlkAssert.AssertEqual("VerifyTableTitle", Convert.ToBoolean(TrueOrFalse), TableData.VerifyTableTitle(TableTitle));
            DlkLogger.LogInfo("VerifyTableTitle() passed, comparing string " + TableTitle + " to table title yields a value of " + TrueOrFalse);
        }
