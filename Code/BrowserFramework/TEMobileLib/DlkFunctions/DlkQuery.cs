...
namespace TEMobileLib.DlkFunctions
{
    ...
    public static class DlkQuery
    {
        ...
        public static void SetSearchCriterion(String CriterionName, String Relationship, String SearchString)
        {
            ...
            {
                // Find CriterionName
                //DlkBaseControl ctl = new DlkBaseControl("Target", "xpath", $"//*[@id='basicQTbl']/tbody/tr/td/span[starts-with(., \"{CriterionName}\")]");
                var ctls = DlkEnvironment.AutoDriver.FindElements(By.XPath("//*[@id='basicQTbl']/tbody/tr/td/span[contains(@id, 'basicFieldLbl')]"));
                if (ctls.Count == 0) throw new Exception("No search criterion found");

                var ctl = ctls.FirstOrDefault(c => Regex.Replace(c.Text, @"\s+", " ").Trim() == Regex.Replace(CriterionName, @"\s+", " ").Trim());

                if(ctl == null) throw new Exception($"Cannot find criterion name [{CriterionName}]");

                DlkBaseControl relationship = new DlkBaseControl("Relationship", ctl.FindElement(By.XPath("../following-sibling::td[1]")));
                DlkBaseControl searchString = new DlkBaseControl("SearchString", ctl.FindElement(By.XPath("../following-sibling::td[2]")));
