...
namespace TEMobileLib.DlkControls
{
    public class DlkComboBoxList : DlkBaseControl
    {
        ...
        //Without verify
        public void SelectItemWithoutVerify(String Value, bool IsStrict = true, bool isMenuMobile = false)
        {
            Initialize();
            try
            {
                DlkComboBoxListItem ctl;

                if(IsStrict)
                {
                    if (isMenuMobile)
                    {
                        ctl = new DlkComboBoxListItem("ItemToSelect", this, "xpath", "./descendant::span[text()='" + Value + "']");
                    }
                    else
                    {
                        ctl = new DlkComboBoxListItem("ItemToSelect", this, "xpath", "./descendant::div[text()='" + Value + "']");
                    }
                }
                else
                {
                    IReadOnlyCollection<IWebElement> matches;
                    if (isMenuMobile)
                    {
                        matches = mElement.FindElements(By.XPath("./descendant::span[contains(text(), '" + Value + "')]"));
                    }
                    else
                    {
                        matches = mElement.FindElements(By.XPath("./descendant::div[contains(text(), '" + Value + "')]"));
                    }
                    if (matches.Count == 0)
                    {
                        throw new Exception();
                    ...
                    else
                    {
                        ...
                        IWebElement matchItem = null;

                        //find the closest match
                        foreach (IWebElement item in matches)
                        {
                            int currDistance = DlkString.LevenshteinDistance(item.Text, Value);
                            if (closestMatch > currDistance)
                            {
                                matchItem = item;
                                closestMatch = currDistance;
                            }
                        }
                        ctl = new DlkComboBoxListItem("ItemToSelect", matchItem);
                    }
                }
                ctl.Select();
            }
