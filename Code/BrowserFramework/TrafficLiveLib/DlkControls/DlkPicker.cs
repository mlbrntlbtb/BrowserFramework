...
namespace TrafficLiveLib.DlkControls
{
    ...
    public class DlkPicker : DlkBaseControl
    {
        ...
        public void VerifyContainValue(String ExpectedValue, String TrueOrFalse)
        {
            try
            {
                bool expectedFound = Convert.ToBoolean(TrueOrFalse);
                string ActualValues = "";
                Initialize();
                IReadOnlyCollection<IWebElement> mItems = mElement.FindElements(By.XPath(mItemsXPath)).ToList();

                foreach (IWebElement mItem in mItems)
                {
                    string value = mItem.FindElement(By.CssSelector("div")).GetAttribute("innerHTML");
                    ActualValues += value + "~";
                }
                ...
                if (ActualValues.Contains(ExpectedValue))
                {
                    if (expectedFound)
                    {
                        DlkLogger.LogInfo("VerifyContainValue() passed : " + ExpectedValue + " found.");
                    }
                    else
                    {
                        throw new Exception("VerifyContainValue() : " + ExpectedValue + " not found. Actual values: [" + ActualValues + "]");
                    }
                }
                else
                {
                    if (expectedFound)
                    {
                        throw new Exception("VerifyContainValue () : " + ExpectedValue + " found.");
                    }
                    else
                    {
                        DlkLogger.LogInfo("VerifyContainValue() passed : " + ExpectedValue + " not found. Actual values: [" + ActualValues + "]");
                    }
                }
            }
            catch (Exception e)
