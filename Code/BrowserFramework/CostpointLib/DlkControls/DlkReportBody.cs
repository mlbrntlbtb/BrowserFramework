...
namespace CostpointLib.DlkControls
{
    ...
    public class DlkReportBody : DlkBaseControl
    {
        ...
        [Keyword("VerifyPartialBodyText", new String[] { "1|text|Expected text|Sample text" })]
        public void VerifyPartialBodyText(String ExpectedText)
        {
            try
            {
                Initialize();
                if (mElement.FindElements(By.XPath("./div/table/tbody/tr[2]")).Count < 1)
                {
                    throw new Exception("Unable to find report body");
                }
                DlkBaseControl reportBodyText = new DlkBaseControl("ReportBodyText", mElement.FindElement(By.XPath("./div/table/tbody/tr[2]")));

                // innertext attrib is used by IE
                String innerTextValue = reportBodyText.GetAttributeValue("innerText");
                // textContent attrib is used by Firefox (what about chrome?)
                String textContent = reportBodyText.GetAttributeValue("textContent");
                // if innerText available use it, otherwise use textContent
                String actualReportBodyText = string.IsNullOrEmpty(innerTextValue) ? textContent : innerTextValue;
                if (!string.IsNullOrEmpty(actualReportBodyText))
                {
                    actualReportBodyText = actualReportBodyText.Trim();
                }
                bool PartOfText = actualReportBodyText.Contains(ExpectedText);
                if (PartOfText)
                {
                    actualReportBodyText = ExpectedText;
                }
                else
                {
                    throw new Exception("Text '" + ExpectedText + "' not found in report body");
                }

                DlkAssert.AssertEqual("VerifyPartialBodyText(): " + mControlName, ExpectedText, actualReportBodyText);
                DlkLogger.LogInfo("VerifyPartialBodyText passed");
            }
            catch (Exception e)
            {
                throw new Exception("VerifyPartialBodyText() failed : " + e.Message, e);
            }
        }
