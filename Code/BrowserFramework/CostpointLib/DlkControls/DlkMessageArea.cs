...
namespace CostpointLib.DlkControls
{
    ...
    public class DlkMessageArea : DlkBaseControl
    {
        ...
        [Keyword("GetVerifyExists", new String[] { "1|text|VariableName|IsExists" })]
        public void GetVerifyExists(string VariableName)
        {
            try
            {
                Initialize();
                ...
                {
                    DlkMessageArea messageArea = new DlkMessageArea("MessageArea", mElement);
                    bool messageAreaExists = messageArea.Exists();

                    if (messageAreaExists)
                    {
                        GetMessages();
                        DlkLogger.LogInfo("Message area found with message: " + string.Join(" ", mlstMessages.ToArray()));
                    }

                    DlkVariable.SetVariable(VariableName, messageArea.Exists().ToString());
                    DlkLogger.LogInfo("GetVerifyExists() successfully executed.");
                }                
            }
            catch (Exception e)
            {
        ...
        {
            Initialize();
            DlkBaseControl btnControl = new DlkBaseControl("Close Button", mElement.FindElement(By.CssSelector(mstrCloseMessageAreaCSS)));
            btnControl.Click();
        }
            ...
            try
            {
                ...
                switch (ButtonCaption.ToLower())
                {
                    ...
                    case "cancel":
                        ...
                        break;
                    case "close":
                        btnControl = mElement.FindElement(By.CssSelector(mstrClickCloseButtonCSS), 1);
                        break;
                    default:
                        break;
                }
        ...
        public void ClickMessagesAreaButtonIfExists(String ButtonCaption)
        {
            try
            {
                ...
                {
                    DlkLogger.LogInfo("ClickMessagesAreaButtonIfExists() successfully executed. Messages Area does not exist.");
                }               
            }
            catch (Exception e)
            {
