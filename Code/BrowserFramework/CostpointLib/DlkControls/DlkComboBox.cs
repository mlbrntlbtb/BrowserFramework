...
namespace CostpointLib.DlkControls
{
    ...
    public class DlkComboBox : DlkBaseControl
    {
        ...
        [Keyword("MultiSelect", new String[] { "1|text|Dropdown Values|Value1~Value2" })]
        public void MultiSelect(String Value)
        {
            try
            {
                ...
                if (Value != "")
                {
                    ...
                    DisplayComboBoxList(values[0], 3);

                    foreach (string val in Value.Split('~'))
                    {
                        mComboBoxList.SelectItemWithoutVerify(val, VerifyAfterSelect);
                        Thread.Sleep(1000);
                    }
                ...
                DlkLogger.LogInfo("Successfully executed MultiSelect()");
            }
            catch (Exception e)
            {
        ...
        [Keyword("SetShowSelected", new String[] { "1|text|Value|True" })]
        public void SetShowSelected(string Value)
        {
            try
            {
                IWebElement comboBoxMenu = GetComboBoxMenu();
                if (comboBoxMenu != null)
                {
                    DlkCheckBox checkBox = new DlkCheckBox("SetShowSelected", comboBoxMenu.FindElement(By.XPath("./input[@class='tCCVFI']")));
                    checkBox.Set(Value);
                    DlkLogger.LogInfo("SetShowSelected() passed");
                }
                else
                    throw new Exception("ComboBox menu not exists.");
            }
            catch (Exception e)
            {
                throw new Exception("SetShowSelected() failed : " + e.Message, e);
            }
        }

        [Keyword("ClickComboBoxMenu", new String[] { "1|ComboBox Menu Caption|Select All" })]
        public void ClickComboBoxMenu(String Caption)
        {
            try
            {
                IWebElement comboBoxMenu = GetComboBoxMenu();
                if (comboBoxMenu != null)
                {
                    DlkBaseControl control = new DlkBaseControl("list", comboBoxMenu.FindElement(By.XPath("./span[contains(text(),'" + Caption + "')]")));
                    control.Click();
                    DlkLogger.LogInfo("ClickComboBoxMenu() passed");
                }
                else
                    throw new Exception("ComboBox menu not exists.");
            }
            ...
            {
                throw new Exception("ClickComboBoxMenu() failed : " + e.Message, e);
            }
        }
        ...
        public String GetText()
        {
            ...
            else  if (mClassName == "tccbimg") // special handling for dropdowns with different XML structure
            {
                DlkBaseControl ctlComboText = new DlkBaseControl("ComboText", mElement.FindElement(By.XPath("./preceding-sibling::*[@class='tCCBT'][1]")));
                ret = ctlComboText.GetValue();
            }
            return ret;
        }

        private IWebElement GetComboBoxMenu()
        {
            Initialize();
            if (DlkEnvironment.AutoDriver.FindElements(By.XPath("//div[@class='tCCBV']")).FirstOrDefault(f => !f.GetAttribute("style").Contains("display: none")) == null)
                mElement.Click();
            ...
            while (true)
            {
                try
                {
                    IWebElement control = DlkEnvironment.AutoDriver.FindElement(By.XPath("(//div[@class='tCCBV']//*[not(contains(@style,'display:none'))]//div[@class='tCCVF'])[1]"));
                    return control;                        
                }
                catch
                {
