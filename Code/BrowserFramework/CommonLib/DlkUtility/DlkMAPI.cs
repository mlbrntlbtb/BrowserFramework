...
namespace CommonLib.DlkUtility
{
    public class DlkMAPI
    {
        ...
        int SendMail(string strSubject, string strBody, int how)
        {
            ...
            if (m_lastError > 1)
            {
                if(m_lastError == 2 || m_lastError == 3)
                {
                    MessageBox.Show("No email platform installed or configured. Please install an email platform to use this feature.", "Send via Email"
