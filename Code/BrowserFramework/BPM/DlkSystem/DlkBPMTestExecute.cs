...
{
    public class DlkBPMTestExecute : DlkTestExecute
    {
        public DlkBPMTestExecute(String Product, String LoginConfig, String TestPath, String TestInstance, String Browser, String KeepBrowserOpen)
        ...
        private void Login()
        {
            ...
            {
                DlkBPMFunctionHandler.ExecuteFunction("Login", "", "Login", new String[] 
                {
                ...
                if (!string.IsNullOrEmpty(mLoginConfigHandler.mDatabase))
                {
                    if (!DlkEnvironment.CustomInfo.Keys.Contains("buildversion"))
                    {
                        DlkEnvironment.CustomInfo.Add("buildversion", new string[] { "Build Version", mLoginConfigHandler.mDatabase });
                    }
                }

                if (!DlkEnvironment.CustomInfo.Keys.Contains("phase"))
                {
                    DlkEnvironment.CustomInfo.Add("phase", new string[] { "Phase", DlkBPMGenericMethods.GetPhaseValue() });
                }

                if (!DlkEnvironment.CustomInfo.Keys.Contains("runtype"))
                {
                    DlkEnvironment.CustomInfo.Add("runtype", new string[] { "Run Type", DlkBPMGenericMethods.GetRunTypeValue() });
                }
            }
        }
            ...
            for (int i = 0; i < mTest.mTestSteps.Count; i++) // for each test step
            {
                try
                {
                    ...
                    else if (execute)
                    {
                        ...
                        {
                            DlkBPMFunctionHandler.ExecuteFunction
                                (
                        ...
                        {
                            DlkBPMKeywordHandler.ExecuteKeyword(
                                mTest.mTestSteps[i].mScreen,
                                mTest.mTestSteps[i].mControl,
                                mTest.mTestSteps[i].mKeyword,
                                mTestParams
