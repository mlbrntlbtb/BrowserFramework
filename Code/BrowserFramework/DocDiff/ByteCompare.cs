...
namespace DocDiff
{
    public class ByteCompare
    {
        ...
        public ByteCompare(String ExpectedFile, String ActualFile)
        {
        ...
        public List<DocDiffCompareDiffRecord> CompareFiles()
        {
            ...
            Boolean bTestResult = FileCompare(mExpected, mActual);
            if (bTestResult)
            {
                if (!mErrorsOnly)
                {
                    iType = 0;
                    DocDiffCompareDiffRecord cdr = new DocDiffCompareDiffRecord(iType, "FileByteComparison", "", mExpected, mActual);
                    AllResults.Add(cdr);
                }
            }
            else
