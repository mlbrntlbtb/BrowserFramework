    ...
    public class DlkToggle : DlkBaseControl
    {
        ...
        public void Set(String Value)
        {
            try
            {
                ...
                {
                    bool bClick = false;
                    if(GetAttributeValue("class").Contains("x-deltektoggle-off") && Value.ToLower().Equals("on"))
                    {
                        //if the currentstate if OFF and the user wants to set the value to select ON,
                        bClick = true;
                    }
                    else if ((GetAttributeValue("class").Contains("x-deltektoggle-on")||GetAttributeValue("class").Contains("x-deltektoggle_yes-on")) && Value.ToLower().Equals("off"))
                    {
        ...
        public void VerifyValue(String ExpectedValue)
        {
            try
            {
                ...
                else if (GetAttributeValue("class").Contains("x-deltektoggle_yes-on") || GetAttributeValue("class").Contains("x-deltektoggle-on"))
                {
                    ActText = "ON";
                }
