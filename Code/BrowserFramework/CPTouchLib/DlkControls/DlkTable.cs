...
{
    [ControlType("Table")]
    public class DlkTable : DlkMobilePicker
    {
        ...
        private List<TableListItem> mRows = new List<TableListItem>();
        ...
        public void SelectRowWithColumnValue(string ColumnName, string Value, string ExactMatch, string OneBasedIndex)
        {
            try
            {
                ...
                var matches = new List<TableListItem>();
                if (exact)
                {
        ...
        private void GetRows(List<string> headers)
        {
            mElement.FindElements(By.XPath(STR_XPATH_ROWS)).ToList()
                .ForEach(x => mRows.Add(new TableListItem(new DlkMobileControl("itm", x), headers)));
    ...
    public class TableListItem
    {
        ...
        public TableListItem(DlkMobileControl control, List<string> headers)
        {
            mControl = control;
            mHeaders = headers;
            PopulateValues();
