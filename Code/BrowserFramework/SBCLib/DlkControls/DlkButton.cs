    ...
    public class DlkButton : DlkBaseControl
    {
        ...
        public new void Click()
        {
            try
            {
                ...
                else if(mControlName.Equals("ChooseFiles"))
                {
        ...
        [Keyword("VerifyHighlighted", new String[] { "1|text|Expected Value|TRUE" })]
        public void VerifyHighlighted(String TrueOrFalse)
        {
            try
            {
                Initialize();
                bool isHighlighted = mElement.GetAttribute("class").ToLower().Contains("selected");
                DlkAssert.AssertEqual("VerifyHighlighted() : ", Convert.ToBoolean(TrueOrFalse), isHighlighted);
                DlkLogger.LogInfo("VerifyHighlighted() passed");
            }
            catch (Exception e)
            {
                throw new Exception("VerifyHighlighted() failed : " + e.Message, e);
            }
        }
