...
namespace TM1WebLib.DlkControls
{
    ...
    public class DlkButton : DlkBaseControl
    {
        ...
        {
            FindElement();
            if (mElement.TagName == "button" || GetAttributeValue("role") == "button" || mElement.TagName == "img")
            {
                return true;
            }
            else if(mElement.TagName == "span")
            {
                try
                {
                    IWebElement parentElement = mElement.FindElement(By.XPath("./ancestor::span[@role='button']"));
                    return true;
                }
        ...
        public new void Click()
        {
            try
            {
                ...
                if (mElement.GetAttribute("role") == "button")
                {
