...
{
    public class CP_Inspector : Inspector
    {
        #region PRIVATE MEMBERS
        private Object node;
        private string[] mQueryComponents = { "basicCountBttn", "basicSaveQ", "basicResetQ", "closeQ", "relationList",
        ...
        private readonly string[] m_ValidClasses = { "bOk", "actBtn", "fCalBtn", "fCalBtn20", "fExpandoBtn", "fBrowseBtn", // button
                                                   "fLaunchBtn", "submitBtn", "submitBtnDisabled", // button
                                                   "submitBtnActive", "submitBtnHover", "goToLbl", // button
                                                   "tCCBF", "tCCB" , "tCCBV", "tCCBImgF", "tCCBImgT", "tCCBT", "tCCBImg", // combo
                                                   "rsTbBtn", "rsTbBtnNormal", "rsTbBtnDisabled", "rsTbBtnHover", "rsTbBtnActive", // form buttons
                                                   "busItem", "deptItem", "navItem", //nav menu
                                                   "fCB", //checkbox
                                                   "modalTabLbl", // tab
                                                   "sbtskLnk", "drillDownPathText", "helpLink", "legalLink", "reset", "resetGoodbye", // link
                                                   ...
                                                   "tbBtnHRpt", "tbBtnHRpt nav", // report toolbar
                                                   ...
                                                   "fdfrq" , "fdf" , "fdfro", "fdfrqnum", "fdfronum", "fdfnum", "fdexpandosmall" , "inputfld",
                                                    ...
                                                    "smplLstBxItm", "fCLB", // conditionlistbox
                                                    "myMnuViewport" ,"myMnuBottomImg" ,"myMnuBottomLbl" //AppMenuView
                                                 };
          #endregion
        ...
        private bool IsProbablyLookUp(string DescriptorValue)
        {
        ...
        public override DlkBaseControl GetAccessedControl(out string ActionType, out string AlertText, out DlkCapturedStep CurrentStep, bool IsVerify, bool IsHotkeyPressed)
        {
            ...
            try
            {
                ...
                node = (Object)((IJavaScriptExecutor)DlkEnvironment.AutoDriver).ExecuteAsyncScript(
                            ...
                            "function nodeMouseDown(event) {" + // mouse down event handler
                                "try{ if (event.which == 1) { event.target.setAttribute(\"actiontype\", \"Click\"); } else if (event.which == 3) { event.target.setAttribute(\"actiontype\", \"RightClick\"); } getTargetElementAndValue(event); }" + 
                                "catch (err) {}" +
                            "}" +
        ...
        public override IWebElement InferTarget(IWebElement Source, string sourceClassName,
            ...
            IWebElement parent3 = null, string parent3Id = "", string parent3Class = "")
        {
            ...
            else if (m_ValidClasses.Any(item => item.ToLower().Equals(classNameSource.ToLower())))
            {
                ...
                try
                {
                    if (IsLoginScreen()) // combo box in login page
                    {
                        if ((classNameSource == "tCCBImg" || classNameSource == "tCCBT")) 
                        {
                            ret = Source.FindElement(By.XPath("./ancestor::span[@class='tCCB'][1]"));
                            newClass = "tCCB";
                            newId = ret.GetAttribute("id");
                        }
                    }
            ...
            else
            {
                ...
                if (m_ValidClasses.Contains(parent1Class))
                {
                    ...
                    else if (parent1Class == "smplLstBxItm")
                    {
                        ret = parent1;
                        newClass = parent1Class;
                        newId = parent2Id;
        ...
        public int GetAncestorFormIndex(DlkBaseControl TargetControl, string className = "", string inferredIndex = "")
        {
            int ret = -1;
        ...
        public string GetTargetLink(List<DlkObjectStoreFileControlRecord> Input, string LinkDescriptor, string LinkText, string popRefId = "")
        {
            string ret = "UNKNOWN";
        ...
        public string GetTargetButton(List<DlkObjectStoreFileControlRecord> Input, string ButtonDescriptor, int AncestorFormIndex, string popRefId = "")
        {
            string ret = "UNKNOWN";
            ...
            // special case of close buttons in login page
            if (relevantID.Equals("closeLegal"))
            {
                string searchstring = "";
                if (targetClass.Equals("hideCriteriaImg"))
                {
                    searchstring = "//*[@id='" + targetClass + "']";
                }
                else if (targetClass.Equals("legalDisclaimer"))
                {
                    searchstring = "//*[@id='hideCriteriaImg']";
                }
                else
                {
                    searchstring = "//div[@class='" + targetClass + "']/*[@class='" + relevantID + "']";
                }
                ret = Input.Find(y => y.mSearchParameters == searchstring).mKey;
            }
            ...
            switch (targetClass)
            {
                ...
                case "showAdditionalCriteria":
                    {
                        string searchstring = "showAdditionalCriteria~ID";
                        ret = Input.Find(y => y.mSearchParameters.Contains(searchstring)).mKey;
                        break;
                    }
                case "showCriteriaImg":
                    {
                        string searchstring = "showCriteriaImg~ID";
                        ret = Input.Find(y => y.mSearchParameters.Contains(searchstring)).mKey;
                        break;
                    }
                ...
                case "bBack":
                    {
                        string searchstring = "//*[@id=\""+ targetClass + "\"]";
                        ret = Input.Find(y => y.mSearchParameters.Contains(searchstring)).mKey;
                        break;
                    }
            }
        ...
        public string GetTargetRadioButton(List<DlkObjectStoreFileControlRecord> Input, string Descriptor, int AncestorFormIndex, string popRefId ="")
        {
            string ret = "UNKNOWN";
            ...
            try
            {
                ...
                // for radio button in login page, refer to id since there is no value attribute
                if (IsLoginScreen())
                {
                    searchstring = "//*[@id='" + relevantId + "']";
                }
        ...
        public string GetTargetTable(List<DlkObjectStoreFileControlRecord> Input, int AncestorFormIndex, DlkBaseControl ctl, string popRefId="")
        {
            string ret = "UNKNOWN";

            try
            {
                DlkBaseControl cellElement = ctl;
                if (!cellElement.mElement.GetAttribute("class").Contains("cll"))
                {
                    cellElement = new DlkBaseControl ("cellElement", ctl.mElement.FindElement(By.XPath("./ancestor::*[contains(@class,'cll')]")));
                }
                bool isRightMultiPart = cellElement.mElement.FindElements(By.XPath("../ancestor::div[contains (@id, 'rightFence')]")).Any();
                string targetForm = GetTargetForm(Input.FindAll(y => y.mControlType == "Form"), AncestorFormIndex, popRefId);
                DlkObjectStoreFileControlRecord obj = Input.Find(y => y.mKey == targetForm);
                string searchstring = obj.mSearchParameters + "/descendant::*[@id='dTbl']";
                if (Input.Any(y => y.mSearchParameters == searchstring && y.mControlType == Constants.CP_CONTROL_TYPE_TABLE) && !isRightMultiPart)
                {
                    ret = Input.Find(y => y.mSearchParameters == searchstring && y.mControlType == Constants.CP_CONTROL_TYPE_TABLE).mKey;
                }
                else
                {
                    ret = Input.Find(y => y.mSearchParameters == searchstring && y.mControlType == Constants.CP_CONTROL_TYPE_MULTIPARTTABLE).mKey;
                }
            }
            catch
            {
        ...
        public string GetTargetControl(List<DlkObjectStoreFileControlRecord> Input, string TextBoxDescriptor, int AncestorFormIndex, string popRefId = "")
        {
            string ret = "UNKNOWN";
        ...
        public string GetTargetForm(List<DlkObjectStoreFileControlRecord> Input, int AncestorFormIndex, string popRefId = "")
        {
            string ret = "UNKNOWN";
            try
            {
        ...
        public string GetTargetFormXPath(int AncestorFormIndex)
        {
            string ret = "";
            IWebElement frm = DlkEnvironment.AutoDriver.FindElement(By.XPath("//div[@id='" + AncestorFormIndex + "']/form[1]"));
            try
        ...
        public override string GetScreen(string DescriptorValue)
        {
            // this is a stub
            string alias;
        ...
        public override KeyValuePair<string, string> GetDescriptor(DlkBaseControl Source, string Id, string Class)
        {
            ...
            switch (ctlClass)
            {
                ...
                case "qrLink":
                    ...
                    break;
                case "closeLegal":
                    IWebElement parent = Source.GetParent();
                    string parentClass = parent.GetAttribute("class");
                    ctlDescriptor = ctlClass + "~" + parentClass;
                    break;
                case "hideCriteriaImg":
                    if (string.IsNullOrEmpty(Source.GetAttributeValue("id")))
                    {
                        IWebElement parentElm = Source.GetParent();
                        string parentClassElm = parentElm.GetAttribute("class");
                        ctlDescriptor = parentClassElm + "~" + ctlClass;
                    }
                    break;
                case "popupClose":
        ...
        public string GetControlValue(DlkBaseControl Target)
        {
            string ret = "";
            try
            {
        ...
        public string GetCalendarDateValue(DlkBaseControl ctl)
        {
        ...
        /// <summary>
        /// Gets the calendar element and returns the selected month value
        /// </summary>
        /// <param name="ctl">The element containing a selected date</param>
        /// <returns>Returns the selected month value</returns>
        public string GetCalendarMonthValue(DlkBaseControl ctl)
        {
            string ret = string.Empty;
            IWebElement calendar = ctl.mElement;
            try
        ...
        public bool GetTableColumnHeader(DlkBaseControl ctl, string className, out string Header)
        {
            bool ret = true;
            Header = string.Empty;
            try
            {
                DlkBaseControl cellElement = ctl;
                string cls = string.IsNullOrEmpty(className) ? cellElement.mElement.GetAttribute("class") : className;
                if (!cls.Contains("cll"))
                {
                    cellElement = new DlkBaseControl ("cellElement", ctl.mElement.FindElement(By.XPath("./ancestor::*[contains(@class,'cll')]")));
                }
                IReadOnlyCollection<IWebElement> siblings = cellElement.mElement.FindElements(By.XPath("./preceding-sibling::*"));
                int numSiblings = siblings.Count(x => x.Displayed);
                DlkBaseControl hdr;
                bool isRightMultiPart = cellElement.mElement.FindElements(By.XPath("../ancestor::div[contains (@id, 'rightFence')]")).Any();
                if (isRightMultiPart)
                {
                    hdr = new DlkBaseControl ("columnHeader", cellElement.mElement.FindElement(By.XPath("../../preceding-sibling::div[@id='rightFenceHdr']/div["
                    + (numSiblings + 1) + "]")));
                }
                else
                {
                    hdr = new DlkBaseControl ("columnHeader", cellElement.mElement.FindElement(By.XPath("./ancestor::*[@id='dTbl']/div[@id='tHdr']/div["
                    + (numSiblings + 1) + "]")));
                }
                Header = new DlkBaseControl("HDR", hdr.mElement).GetValue().Trim();
            }
            catch
            {
                ret = false;
        ...
        public string GetMessages(DlkBaseControl msg)
        {
        ...
        public bool GetTableRowNumber(DlkBaseControl ctl, out string Row)
        {
            bool ret = true;
            Row = string.Empty;
            try
            {
                DlkBaseControl cellElement = ctl;
                if (!cellElement.GetAttributeValue("class").Contains("cll"))
                {
                    cellElement = new DlkBaseControl ("cellElement", cellElement.mElement.FindElement(By.XPath("./ancestor::*[contains(@class,'cll')]")));
                }

                DlkBaseControl rowElement = new DlkBaseControl ("rowElement", cellElement.mElement.FindElement(By.XPath("./..")));
                string rowId = rowElement.mElement.GetAttribute("id");
                int count = 0;
                List<IWebElement> rows;
                bool isRightMultiPart = cellElement.mElement.FindElements(By.XPath("../ancestor::div[contains (@id, 'rightFence')]")).Any();
                rows = isRightMultiPart ? cellElement.mElement.FindElements(By.XPath("../../descendant::*[@class='dRw']")).ToList() : cellElement.mElement.FindElements(By.XPath("./ancestor::*[@id='dTbl']/descendant::*[@class='dRw']")).ToList();
