...
{
    public class DlkSFTTestExecute : DlkTestExecute
    {
        public DlkSFTTestExecute(String Product, String LoginConfig, String TestPath, String TestInstance, String Browser, String KeepBrowserOpen)
        ...
        public override void TestSetup()
        {
            try
            {
                if (DlkEnvironment.mBrowser == "IE")
                {
                    ...
                    //If IE webdriver does not initialize properly, retry once
                    if (!InitializeIE(out error))
                    {
                        //Close any open webdriver sessions first before re-initializing IEDriver
                        DlkLogger.LogWarning("WebDriver failed to initialize IE browser. Retrying...");
                        TerminateIEWebdriver();
                        TerminateIEProcesses();
            ...
            for (int i = 0; i < mTest.mTestSteps.Count; i++) // for each test step
            {
                try
                {
                    ...
                    else if (execute)
                    {
                        ...
                        {
                            DlkSFTFunctionHandler.ExecuteFunction
                                (
                        ...
                        {
                            DlkSFTKeywordHandler.ExecuteKeyword(
                                mTest.mTestSteps[i].mScreen,
                                mTest.mTestSteps[i].mControl,
                                mTest.mTestSteps[i].mKeyword,
                                mTestParams
        ...
        /// <summary>
        /// Initializes Internet Explorer with capabalities that enable interaction with SFT controls.
        /// </summary>
        private bool InitializeIE(out string errorMsg)
        {
            ...
            try
            {
                ...
                ieCapabilities.UnhandledPromptBehavior = OpenQA.Selenium.UnhandledPromptBehavior.Accept;
                
                DlkEnvironment.AutoDriver = new InternetExplorerDriver(ieCapabilities);
        ...
        private void Login()
        {
            /* No autologin if User and Password are BOTH blank */
            DlkSFTFunctionHandler.ExecuteFunction("Login", "", "Login", new String[]
            {
                    mLoginConfigHandler.mUrl,
                    mLoginConfigHandler.mUser,
                    mLoginConfigHandler.mPassword,
                    mLoginConfigHandler.mDatabase
            });
        }

        private static void TerminateIEProcesses()
        {
            if (DlkEnvironment.ProcessExists("iexplore") && !DlkEnvironment.IsBrowserOpen)
                DlkEnvironment.KillProcessByName("iexplore");//closes any internet explorer instance
        }

        private static void TerminateIEWebdriver()
        {
