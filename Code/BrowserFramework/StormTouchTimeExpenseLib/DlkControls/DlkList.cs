    ...
    public class DlkList : DlkBaseControl
    {
        ...
        public void Select(String Value)
        {
            try
            {
                ...
                for (int i = 0; i < this.iFindElementDefaultSearchMax; i++)
                {
                    try
                    {
                        if (mElement.FindElements(By.XPath(".//div[contains(@class, 'x-list-item')]//div[@class='x-innerhtml'][contains(., '" + Value + "')]")).Count > 0)
                            mSelected = mElement.FindElement(By.XPath(".//div[contains(@class, 'x-list-item')]//div[@class='x-innerhtml'][contains(., '" + Value + "')]"));
                        else
                            mSelected = mElement.FindElement(By.XPath(".//div[contains(@class, 'x-listitem')]//div[@class='x-innerhtml'][contains(., '" + Value + "')]"));
        ...
        public void SetToggleStateByRow(String Row, String State)
        {
            try
            {
                ...
                string itemsXpath = string.Format(".//div[contains(@class,'list-item-relative') and contains(@id,'simplelistitem')][{0}]", zeroBasedRowIndex);
                for (int i = 0; i < this.iFindElementDefaultSearchMax; i++)
                {
                    try
                    {
