...
namespace METestHarness.Sys
{
    ...
    public static class MSUIAutomationHelper
    {
                                        ...
                                        string controlType = "")
        {
            ...
            if (window != null)
            {
                Driver.SessionLogger.WriteLine(string.Format("{0} found. Handle: 0x{1}.", window.Current.Name, window.Current.NativeWindowHandle.ToString("X")), Logger.MessageType.INF);
            }
            else
            {
                Driver.SessionLogger.WriteLine(string.Format(" Can't find window that contains {0}.", keywordSearch), Logger.MessageType.INF);
            }
                                ...
                                string keywordSearch)
        {
            ...
            {
                Driver.SessionLogger.WriteLine(string.Format(" Can't find window/s that contains {1} in {0}.", root.Current.Name, keywordSearch), Logger.MessageType.INF);
            }
        ...
        public static AutomationElement GetFirstWindow(string windowTitle, string controlType, string value)
        {
            ...
            List<AutomationElement> windows = MSUIAutomationHelper.FindAllWindow(aeDesktop, controlWalker, windowTitle);
        ...
        private static void FirefoxDownload(String Filename, int iWaitTimeInSecs, String action = "Save")
        {
            ...
            AutomationElement govWinWindow = MSUIAutomationHelper.FindWindow(aeDesktop, controlWalker, MSUIAutomationHelper.GovWinIQKeyword);

            if (govWinWindow == null)
            {
                govWinWindow = MSUIAutomationHelper.FindWindow(aeDesktop, controlWalker, "Mozilla Firefox");
            }

            Thread.Sleep(1000);

            AutomationElement openingDialog = MSUIAutomationHelper.FindWindow(govWinWindow, controlWalker, "Opening");
            ...
            govWinWindow = MSUIAutomationHelper.FindWindow(aeDesktop, controlWalker, MSUIAutomationHelper.GovWinIQKeyword);

            if (action == "Open")
            {
                {
                    govWinWindow = MSUIAutomationHelper.FindWindow(aeDesktop, controlWalker, "Microsoft Excel");
                }

                AutomationElement xlsDialog = MSUIAutomationHelper.FindWindow(govWinWindow, controlWalker, "Microsoft Excel");
                ...
                Driver.SessionLogger.WriteLine("Successfully executed FileOpen(): " + Filename, Logger.MessageType.INF);
            }
            else
            {
                if (govWinWindow == null)
                ...
                AutomationElement filenameDialog = MSUIAutomationHelper.FindWindow(govWinWindow, controlWalker, "Enter name");
                ...
                AutomationElement confirmDialog = MSUIAutomationHelper.FindWindow(filenameDialog, controlWalker, "Confirm");
                ...
                Driver.SessionLogger.WriteLine("Successfully executed FileDownload(): " + Filename, Logger.MessageType.INF);
            }

        }

        public static void FileUpload(String BrowserTitle, String Filename, int iWaitTimeInSecs)
        {
            ...
            {
                browserWindow = MSUIAutomationHelper.FindWindow(aeDesktop, controlWalker, BrowserTitle);
            ...
            ValuePattern fNameEditBox = (ValuePattern)fNameEditBoxElement.GetCurrentPattern(ValuePattern.Pattern);
            Driver.SessionLogger.WriteLine("Filename: " + Filename, Logger.MessageType.INF);
            ...
            if (!bClicked)
            {
                Driver.SessionLogger.WriteLine("FileUpload() failed. Unable to click 'Open', will now try to use keyboard event.", Logger.MessageType.INF);
            ...
            {
                Driver.SessionLogger.WriteLine("FileUpload() : 'Open' clicked.", Logger.MessageType.INF);
            }
            //OpenButton.SetFocus();
            ...
            Driver.SessionLogger.WriteLine("Successfully executed FileUpload(): " + Filename, Logger.MessageType.INF);
        }
        ...
        public static void MultipleFileUpload(String BrowserTitle, String FileList, int iWaitTimeInSecs)
        {
            try
            {
                ...
                Driver.SessionLogger.WriteLine("File names: " + constructedList, Logger.MessageType.INF);
                ...
                if (!bClicked)
                {
                    Driver.SessionLogger.WriteLine("MultipleFileUpload() failed. Unable to click 'Open', will now try to use keyboard event.", Logger.MessageType.INF);
                    System.Threading.Thread.Sleep(1000); // wait, then check if any upload dialog still exists
                    uploadDialogList.Clear();
                    uploadDialogList = GetDialogInfo(browserWindow, controlWalker);
                    if (uploadDialogList.Count(x => x != null) > 0)
                ...
                {
                    Driver.SessionLogger.WriteLine("MultipleFileUpload() : 'Open' clicked.", Logger.MessageType.INF);
                }
                System.Threading.Thread.Sleep(1000);

                Driver.SessionLogger.WriteLine("Successfully executed MultipleFileUpload(): " + FileList, Logger.MessageType.INF);
            }
            catch (Exception ex)
            {
        ...
        private static List<AutomationElement> GetDialogInfo(AutomationElement window, TreeWalker walker, string controlType = "")
        {
            ...
            dialogList.Add(MSUIAutomationHelper.FindWindow(window, walker, "Upload", controlType));
            dialogList.Add(MSUIAutomationHelper.FindWindow(window, walker, "Open", controlType));
            dialogList.Add(MSUIAutomationHelper.FindWindow(window, walker, "Choose File to Upload", controlType));
            return dialogList;
        }
        ...
        public static String GetBrowserStatusBar(String BrowserTitle, String iWaitTimeInSecs)
        {
            ...
            Driver.SessionLogger.WriteLine("Successfully executed GetBrowserStatusBar().", Logger.MessageType.INF);
