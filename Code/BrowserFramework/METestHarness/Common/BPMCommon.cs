...
namespace METestHarness.Common
{
    public static class BPMCommon
    {
        ...
        public static void Login(string EnvId)
        {
            try
            {
                /* Determine env */
                ...
                /* Set username */
                Control userName = new Control("userName", "iframe_xpath", "servletBridgeIframe_//input[contains(@id,'USERNAME')]");
                userName.SendKeys(myEnv.UserName);
                Driver.Instance.SwitchTo().DefaultContent();

                /* Set password */
                Control passWord = new Control("passWord", "iframe_xpath", "servletBridgeIframe_//input[contains(@id,'PASSWORD')]");
                passWord.SendKeys(myEnv.Password);
                Driver.Instance.SwitchTo().DefaultContent();

                /* Click login button */
                Control loginButton = new Control("loginBtn", "iframe_xpath", "servletBridgeIframe_//input[contains(@id,'logonButton')]");
                loginButton.Click();
                Driver.Instance.SwitchTo().DefaultContent();
            }
            catch
        ...
        public static void VerifyAlertText(String ExpectedAlertText)
        {
            ...
            AssertEqual(ExpectedAlertText, ActAlertText);
        }
        ...
        public static Environment[] Environments = new Environment[]
        {
                ...
                {
                    Id = "default",
                    Url = "http://10.4.9.114:8080/BOE/BI",
